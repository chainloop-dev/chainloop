...
func (r *Remote) Write(ctx context.Context, key string, state *crafter.VersionedState) error {
	if state == nil {
	...
	if _, err := r.client.Save(ctx, &pb.AttestationStateServiceSaveRequest{WorkflowRunId: key, AttestationState: state.CraftingState, BaseDigest: state.Digest}); err != nil {
...
func (r *Remote) Read(ctx context.Context, key string, state *crafter.VersionedState) error {
	if state == nil {
		return fmt.Errorf("state cannot be nil")
	} else if key == "" {
		return fmt.Errorf("key cannot be empty")
	...
	state.Digest = resp.Result.GetDigest()
