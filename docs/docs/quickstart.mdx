...
5. Create a sample payload to attest.
    ```bash
    cat "hello chainloop" > hello.txt
    ```

6. Sign the payload and push the attestation to Chainloop
    ```bash
    chainloop att init --workflow-name mywf
    chainloop att add --value hello.txt
    chainloop att push
    ```
    Here we are: [initiating an attestation](/getting-started/attestation-crafting#initialization), [adding materials to it](/getting-started/attestation-crafting#adding-materials), and finally [signing and pushing the signature](/getting-started/attestation-crafting#encode-sign-and-push-attestation) to Chainloop
