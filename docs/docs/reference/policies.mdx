...
> NOTE: Policies output format is evolving. Although the example below is still valid, in future Chainloop versions, policy output will be required to provide additional fields. Check [this how-to](rego-how-to.mdx) to start using the new format.
