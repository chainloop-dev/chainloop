...
func TestNewGitleaksReportCrafter(t *testing.T) {
	testCases := []struct {
		name    string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := materials.NewGitleaksReportCrafter(tc.input, nil, nil)
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestGitleaksReportCrafter_Craft(t *testing.T) {
	testCases := []struct {
		name        string
		filePath    string
		wantErr     string
