	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(got.GetArtifact(), &attestationApi.Attestation_Material_Artifact{
				Digest: "sha256:fe88fdaf51a4f33029488ade0c2d93620d5676c51833102e139fbe1fbc7694db", Name: "ghas-code-scan.json",
			})
		})
	}
}
...
func TestGHASSecretScanCraft(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// // The result includes the digest reference
			assert.Equal(got.GetArtifact(), &attestationApi.Attestation_Material_Artifact{
				Digest: "sha256:603aecf4c64b6b448f68870bf043c4b349189d5e7e524e6ddddbc0b12f2679ef", Name: "ghas-secret-scan.json",
			})
		})
	}
}
...
func TestGHASDependencyScanCraft(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// // The result includes the digest reference
			assert.Equal(got.GetArtifact(), &attestationApi.Attestation_Material_Artifact{
				Digest: "sha256:7ff29af4f017d77127883e0a15d19f69a5bbf75e224a44f0f475c155380f0dc2", Name: "ghas-dependency-scan.json",
			})
		})
	}
}
