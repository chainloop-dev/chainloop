...
func (pgv *PolicyGroupVerifier) VerifyMaterial(ctx context.Context, material *api.Attestation_Material, path string, content []byte) ([]*api.PolicyEvaluation, error) {
	...
	for _, groupAtt := range groupAtts {
		...
		subject, err := material.GetEvaluableContent(path, content)
		if err != nil {
			return nil, NewPolicyError(err)
		}
		...
		for i, policyAtt := range policyAtts {
			g.Go(func() error {
				...
				ev, err := pgv.evaluatePolicyAttachment(gCtx, applyGroupGate(policyAtt, groupAtt), subject,
	...
	for _, groupAtt := range attachments {
		...
		for i, attachment := range attestationPolicies {
			g.Go(func() error {
				...
				ev, err := pgv.evaluatePolicyAttachment(gCtx, applyGroupGate(attachment, groupAtt), statementJSON,
