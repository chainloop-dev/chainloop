...
Refer to https://docs.chainloop.dev/guides/custom-policies for more information.
...
```
--claude            install Claude Code hooks (default when no harness flag is set)
...
--workflow string   chainloop workflow name used for trace attestations (defaults to "ai-coding-session")
```
...
Run a single-shot AI coding session and attest it

Synopsis

Wrap an external command (typically an AI coding agent) and emit a
