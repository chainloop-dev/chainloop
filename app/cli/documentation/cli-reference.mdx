...
--policy-input-from-file-replace stringArray   like --policy-input-from-file but the extracted values REPLACE (override) any contract-declared value for the input instead of being appended to it; same [<policy>:]<input>=<file>[:<column>] format; repeatable. The file is also recorded as EVIDENCE.
