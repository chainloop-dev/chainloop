...
func (_q *ProjectQuery) QueryVersions() *ProjectVersionQuery {
...
func (_q *ProjectQuery) WithVersions(opts ...func(*ProjectVersionQuery)) *ProjectQuery {
	query := (&ProjectVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) loadVersions(ctx context.Context, query *ProjectVersionQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectVersion)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
