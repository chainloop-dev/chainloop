...
func (uq *UserQuery) QueryGroupUsers() *GroupMembershipQuery {
...
func (uq *UserQuery) WithGroupUsers(opts ...func(*GroupMembershipQuery)) *UserQuery {
	query := (&GroupMembershipClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGroupUsers; query != nil {
		if err := uq.loadGroupUsers(ctx, query, nodes,
			func(n *User) { n.Edges.GroupUsers = []*GroupMembership{} },
			func(n *User, e *GroupMembership) { n.Edges.GroupUsers = append(n.Edges.GroupUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadGroupUsers(ctx context.Context, query *GroupMembershipQuery, nodes []*User, init func(*User), assign func(*User, *GroupMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
