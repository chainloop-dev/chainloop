...
type Organization struct {
	...
	// DisableRequirementsAutoMatching disables automatic matching of policies to requirements
...
type OrganizationRepo interface {
	...
	Update(ctx context.Context, id uuid.UUID, blockOnPolicyViolation *bool, policiesAllowedHostnames []string, preventImplicitWorkflowCreation *bool, restrictContractCreationToOrgAdmins *bool, disableRequirementsAutoMatching *bool) (*Organization, error)
...
func NewOrganizationUseCase(repo OrganizationRepo, repoUC *CASBackendUseCase, auditor *AuditorUseCase, iUC *IntegrationUseCase, mRepo MembershipRepo, onboardingConfig []*config.OnboardingSpec, l log.Logger) *OrganizationUseCase {
	...
	uc := &OrganizationUseCase{orgRepo: repo,
	...
	if len(onboardingConfig) > 0 {
		for _, spec := range onboardingConfig {
			uc.logger.Infow("msg", "Auto-onboarding enabled", "org", spec.GetName(), "role", spec.GetRole())
		}
	}
...
func (uc *OrganizationUseCase) Update(ctx context.Context, userID, orgName string, blockOnPolicyViolation *bool, policiesAllowedHostnames []string, preventImplicitWorkflowCreation *bool, restrictContractCreationToOrgAdmins *bool, disableRequirementsAutoMatching *bool) (*Organization, error) {
	...
	// Perform the update
	org, err := uc.orgRepo.Update(ctx, orgUUID, blockOnPolicyViolation, policiesAllowedHostnames, preventImplicitWorkflowCreation, restrictContractCreationToOrgAdmins, disableRequirementsAutoMatching)
	if err != nil {
...
func (uc *OrganizationUseCase) AutoOnboardOrganizations(ctx context.Context, userID string) error {
	...
	for _, spec := range uc.onboardingConfig {
		...
		if err != nil {
			...
			uc.logger.Infow("msg", "can't auto-onboard to organization, not found", "name", spec.GetName())
			continue
		}

		// Ensure user membership
		if err := uc.ensureUserMembership(ctx, org, userUUID, PbRoleToBiz(spec.GetRole())); err != nil {
...
func (uc *OrganizationUseCase) ensureUserMembership(ctx context.Context, org *Organization, userUUID uuid.UUID, role authz.Role) error {
	orgUUID, err := uuid.Parse(org.ID)
	if err != nil {
		return NewErrInvalidUUID(err)
	}
	...
	uc.logger.Infow("msg", "User auto-onboarded to organization", "orgName", org.Name, "orgID", orgUUID, "user", userUUID, "role", role)
	return nil
}
