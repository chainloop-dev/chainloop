...
func (s *OrgIntegrationTestSuite) TestUpdate() {
	...
	s.Run("org non existent", func() {
		// org not found
		_, err := s.Organization.Update(ctx, s.user.ID, uuid.NewString(), nil, nil, nil, nil, nil)
		s.Error(err)
	...
	s.Run("org not accessible to user", func() {
		...
		require.NoError(s.T(), err)
		_, err = s.Organization.Update(ctx, s.user.ID, org2.Name, nil, nil, nil, nil, nil)
		s.Error(err)
		s.True(biz.IsNotFound(err))
	})

	s.Run("valid block on policy violation update", func() {
		got, err := s.Organization.Update(ctx, s.user.ID, s.org.Name, toPtrBool(true), nil, nil, nil, nil)
		s.NoError(err)
		s.True(got.BlockOnPolicyViolation)
	})

	s.Run("valid policy allowed hostnames update", func() {
		got, err := s.Organization.Update(ctx, s.user.ID, s.org.Name, nil, []string{"foo.com", "bar.com"}, nil, nil, nil)
		s.NoError(err)
		s.Equal([]string{"foo.com", "bar.com"}, got.PoliciesAllowedHostnames)
	})

	s.Run("clear policy allowed hostnames", func() {
		got, err := s.Organization.Update(ctx, s.user.ID, s.org.Name, nil, []string{}, nil, nil, nil)
		s.NoError(err)
	...
	s.Run("but not passing a value doesn't clear the hostnames value", func() {
		...
		got, err = s.Organization.Update(ctx, s.user.ID, s.org.Name, nil, nil, nil, nil, nil)
		s.NoError(err)
		s.Equal([]string{"foo.com", "bar.com"}, got.PoliciesAllowedHostnames)
	})
}
