...
// UpdateValidationStatus provides a mock function with given fields: ctx, ID, status, validationError
func (_m *CASBackendRepo) UpdateValidationStatus(ctx context.Context, ID uuid.UUID, status biz.CASBackendValidationStatus, validationError *string) error {
	ret := _m.Called(ctx, ID, status, validationError)
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID, biz.CASBackendValidationStatus, *string) error); ok {
		r0 = rf(ctx, ID, status, validationError)
	} else {
		r0 = ret.Error(0)
	}
