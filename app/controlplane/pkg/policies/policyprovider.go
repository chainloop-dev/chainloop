...
func (p *PolicyProvider) Resolve(policyName string, token string) (*schemaapi.Policy, string, error) {
	if policyName == "" || token == "" {
...
func (p *PolicyProvider) resolveRef(policyName, digest string) (string, error) {
	...
	if uri.Host == "" {
		return "", fmt.Errorf("invalid policy provider URL")
	}

	// Construct the URL for the policy
	res := fmt.Sprintf("chainloop://%s/%s", uri.Host, policyName)
	// including digest if needed
	if digest != "" {
		res = fmt.Sprintf("%s@%s", res, digest)
	}
