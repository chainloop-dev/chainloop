...
type Bootstrap struct {
	...
	// Outbound network policy applied to the integration plugins
	PluginsNetworkPolicy *PluginsNetworkPolicy `protobuf:"bytes,22,opt,name=plugins_network_policy,json=pluginsNetworkPolicy,proto3" json:"plugins_network_policy,omitempty"`
...
func (x *Bootstrap) GetPluginsNetworkPolicy() *PluginsNetworkPolicy {
	if x != nil {
		return x.PluginsNetworkPolicy
	}
	return nil
}

type PluginsNetworkPolicy struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Refuse to connect to destinations that are not publicly routable
	// (loopback, private ranges, cloud metadata endpoints) from the plugins
	// that accept an arbitrary target URL, currently the generic webhook and
	// Dependency-Track plugins.
	//
	// It defaults to false so that deployments running those services inside
	// their own network keep working. Plugins whose destination is a known
	// public service, such as the Slack and Discord webhooks, always refuse
	// non-public destinations regardless of this setting.
	BlockPrivateTargets bool `protobuf:"varint,1,opt,name=block_private_targets,json=blockPrivateTargets,proto3" json:"block_private_targets,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *PluginsNetworkPolicy) Reset() {
	*x = PluginsNetworkPolicy{}
	mi := &file_controlplane_config_v1_conf_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PluginsNetworkPolicy) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PluginsNetworkPolicy) ProtoMessage() {}

func (x *PluginsNetworkPolicy) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_config_v1_conf_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use PluginsNetworkPolicy.ProtoReflect.Descriptor instead.
func (*PluginsNetworkPolicy) Descriptor() ([]byte, []int) {
	return file_controlplane_config_v1_conf_proto_rawDescGZIP(), []int{1}
}

func (x *PluginsNetworkPolicy) GetBlockPrivateTargets() bool {
	if x != nil {
		return x.BlockPrivateTargets
	}
	return false
}
...
var file_controlplane_config_v1_conf_proto_goTypes = []any{
	...
	(*PluginsNetworkPolicy)(nil),            // 1: controlplane.config.v1.PluginsNetworkPolicy
...
var file_controlplane_config_v1_conf_proto_depIdxs = []int32{
	...
	10, // 6: controlplane.config.v1.Bootstrap.certificate_authority:type_name -> controlplane.config.v1.CA
	10, // 7: controlplane.config.v1.Bootstrap.certificate_authorities:type_name -> controlplane.config.v1.CA
	9,  // 8: controlplane.config.v1.Bootstrap.timestamp_authorities:type_name -> controlplane.config.v1.TSA
	25, // 9: controlplane.config.v1.Bootstrap.onboarding:type_name -> controlplane.config.v1.OnboardingSpec
	11, // 10: controlplane.config.v1.Bootstrap.prometheus_integration:type_name -> controlplane.config.v1.PrometheusIntegrationSpec
	5,  // 11: controlplane.config.v1.Bootstrap.policy_providers:type_name -> controlplane.config.v1.PolicyProvider
	14, // 12: controlplane.config.v1.Bootstrap.nats_server:type_name -> controlplane.config.v1.Bootstrap.NatsServer
	4,  // 13: controlplane.config.v1.Bootstrap.federated_authentication:type_name -> controlplane.config.v1.FederatedAuthentication
