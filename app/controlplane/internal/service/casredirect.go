...
func (s *CASRedirectService) HTTPDownloadRedirectHandler(ctx khttp.Context) error {
	...
	// Authentication dance if t is not present
	userToken := ctx.Request().URL.Query().Get("t")
	// redirect to login
	if userToken == "" {
		loginURL, err := url.Parse(AuthLoginPath)
		if err != nil {
			return err
		}
		...
		q := loginURL.Query()
		q.Set("callback", ctx.Request().URL.String())
		loginURL.RawQuery = q.Encode()

		// Get the current user auth token and set it in the request context
		http.Redirect(ctx.Response(), ctx.Request(), loginURL.String(), http.StatusTemporaryRedirect)
		return nil
	}

	// Set the token in the request, this is required to call GetDownloadURL
	ctx.Request().Header.Set(authorizationKey, fmt.Sprintf(bearerFormat, userToken))
