...
type dispatchQueue struct {
	// List of materials that need to be dispatched
	materials materialsDispatch
	// List of attestation that need to be dispatched
...
func (d *Dispatcher) calculateDispatchQueue(ctx context.Context, orgID, workflowID string) (*dispatchQueue, error) {
	...
	for _, attachment := range attachments {
		...
		if err != nil {
			return nil, fmt.Errorf("finding integration backend: %w", err)
		}
...
func (d *Dispatcher) Run(ctx context.Context, envelope *dsse.Envelope, orgID, workflowID, downloadSecretName string) error {
	...
	for _, i := range q.attestations {
		...
		go dispatch(ctx, i.backend, opts, d.log)
	}
	...
	for _, material := range predicate.GetMaterials() {
		...
		if b := q.materials[schemaapi.CraftingSchema_Material_MATERIAL_TYPE_UNSPECIFIED]; b != nil {
			backends = append(backends, b...)
		}

		if b := q.materials[schemaapi.CraftingSchema_Material_MaterialType(schemaapi.CraftingSchema_Material_MaterialType_value[material.Type])]; b != nil {
			backends = append(backends, b...)
		}
		...
		for _, b := range backends {
			...
			go dispatch(ctx, b.backend, opts, d.log)
